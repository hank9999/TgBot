from libs.MCRcon import rcon
from libs.Status import getStatus
from libs.Message import ReplyMessage
from Setting import TgSetting, RconSetting, ServerList


async def helpCommand(message_id, session):
    text = (
        '帮助:\n'
        '/say 发送消息至MC服务器\n'
        '/run 远程执行指令\n'
    ...
    await ReplyMessage(text, message_id, session)
    return


async def listCommand(message_id, session):
    text = await getStatus(ServerList)
    await ReplyMessage(text, message_id, session)
    return


async def runCommand(message, message_id, session):
...
async def sayCommand(message, message_id, session):
    ...
    await ReplyMessage('已发送', message_id, session)
    return


async def parser(data, session):
    print(data)
    ...
    if need_execute:
        if not is_bot:
            if text.find('/help') == 0:
                await helpCommand(message_id, session)
            elif text.find('/list') == 0:
                await listCommand(message_id, session)
            elif text.find('/run') == 0:
                await runCommand(text, message_id, session)
            elif text.find('/say') == 0:
                await sayCommand(text, message_id, session)
